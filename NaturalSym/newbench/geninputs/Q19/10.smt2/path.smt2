
(set-logic QF_ASNIA)
(set-option :produce-models true)
(set-option :strings-exp true)


(define-fun isinteger ((x!1 String)) Bool (str.in_re x!1 (re.+ (re.range "0" "9")))  )
(define-fun notinteger ((x!1 String)) Bool (not (isinteger x!1)) )
(define-fun real.str.from_int ((i Int)) String (ite (< i 0) (str.++ "-" (str.from_int (- i))) (str.from_int i)))
(define-fun real.str.to_int ((i String)) Int (ite (= (str.substr i 0 1) "-") (- (str.to_int (str.substr i 1 (- (str.len i) 1)))) (str.to_int i)))

(declare-fun input3_d7 () String)
(declare-fun input6_d9 () String)
(declare-fun input6_d2 () String)
(declare-fun input5_d7 () String)
(declare-fun input6_d4 () String)
(declare-fun input6_d8 () String)
(declare-fun input3_d11 () String)
(declare-fun input3_d8 () String)
(declare-fun input2 () String)
(declare-fun input5_d4 () String)
(declare-fun input4_d0 () String)
(declare-fun input6_d13 () String)
(declare-fun input2_d10 () String)
(declare-fun input6_d16 () String)
(declare-fun input4 () String)
(declare-fun input6_d24 () String)
(declare-fun input2_d13 () String)
(declare-fun x3 () String)
(declare-fun input3_d15 () String)
(declare-fun input3_d1 () String)
(declare-fun input1_d4 () String)
(declare-fun input1_d5 () String)
(declare-fun input1 () String)
(declare-fun input2_d12 () String)
(declare-fun input1_d6 () String)
(declare-fun input2_d6 () String)
(declare-fun x52 () String)
(declare-fun input6_d15 () String)
(declare-fun input2_d0 () String)
(declare-fun input1_d2 () String)
(declare-fun input6_d6 () String)
(declare-fun input2_d5 () String)
(declare-fun input3_d10 () String)
(declare-fun input1_d1 () String)
(declare-fun input6_d5 () String)
(declare-fun input3_d6 () String)
(declare-fun input5_d2 () String)
(declare-fun input1_d3 () String)
(declare-fun input6 () String)
(declare-fun input3_d5 () String)
(declare-fun input2_d18 () String)
(declare-fun input6_d21 () String)
(declare-fun x16 () String)
(declare-fun input3 () String)
(declare-fun input6_d22 () String)
(declare-fun input6_d18 () String)
(declare-fun input2_d19 () String)
(declare-fun input3_d13 () String)
(declare-fun input6_d7 () String)
(declare-fun x9 () String)
(declare-fun input6_d3 () String)
(declare-fun input5_d3 () String)
(declare-fun input3_d18 () String)
(declare-fun input3_d16 () String)
(declare-fun input2_d11 () String)
(declare-fun input2_d22 () String)
(declare-fun input3_d3 () String)
(declare-fun input3_d17 () String)
(declare-fun input3_d19 () String)
(declare-fun input3_d12 () String)
(declare-fun input1_d8 () String)
(declare-fun input2_d8 () String)
(declare-fun x6 () String)
(declare-fun input6_d17 () String)
(declare-fun input6_d14 () String)
(declare-fun input5_d9 () String)
(declare-fun input2_d9 () String)
(declare-fun input6_d23 () String)
(declare-fun input5_d1 () String)
(declare-fun input5_d5 () String)
(declare-fun input4_d1 () String)
(declare-fun input2_d2 () String)
(declare-fun input5_d8 () String)
(declare-fun input5_d0 () String)
(declare-fun input4_d3 () String)
(declare-fun input6_d10 () String)
(declare-fun input3_d14 () String)
(declare-fun input3_d0 () String)
(declare-fun input2_d15 () String)
(declare-fun input6_d20 () String)
(declare-fun x41 () String)
(declare-fun input5_d6 () String)
(declare-fun input2_d17 () String)
(declare-fun input6_d0 () String)
(declare-fun input3_d2 () String)
(declare-fun input6_d12 () String)
(declare-fun input2_d7 () String)
(declare-fun input4_d2 () String)
(declare-fun x38 () String)
(declare-fun input2_d14 () String)
(declare-fun input5 () String)
(declare-fun input4_d4 () String)
(declare-fun input2_d21 () String)
(declare-fun input2_d1 () String)
(declare-fun x29 () String)
(declare-fun x26 () String)
(declare-fun input6_d11 () String)
(declare-fun input2_d16 () String)
(declare-fun input3_d4 () String)
(declare-fun input6_d25 () String)
(declare-fun input2_d3 () String)
(declare-fun input2_d20 () String)
(declare-fun input3_d20 () String)
(declare-fun input3_d9 () String)
(declare-fun input1_d7 () String)
(declare-fun input2_d4 () String)
(declare-fun x19 () String)
(declare-fun input6_d19 () String)
(declare-fun input1_d0 () String)
(declare-fun input6_d1 () String)



     
(declare-fun input3_d21 () String)
(declare-fun input6_d26 () String)
(declare-fun input6_d27 () String)
(declare-fun input6_d28 () String)
(declare-fun input5_d10 () String)
(declare-fun input5_d11 () String)
(declare-fun input5_d12 () String)
(declare-fun input4_d5 () String)
(declare-fun input4_d6 () String)
(declare-fun input4_d7 () String)
(declare-fun input4_d8 () String)
(declare-fun input4_d9 () String)
(declare-fun input4_d10 () String)
(declare-fun input4_d11 () String)
(declare-fun input4_d12 () String)
(declare-fun input4_d13 () String)
(declare-fun input4_d14 () String)
(declare-fun input4_d15 () String)
(declare-fun input4_d16 () String)
(declare-fun input4_d17 () String)
(declare-fun input1_d9 () String)
(declare-fun input1_d10 () String)
(declare-fun input1_d11 () String)
(declare-fun input1_d12 () String)
(declare-fun input1_d13 () String)
(declare-fun input1_d14 () String)
(declare-fun input1_d15 () String)
(declare-fun input1_d16 () String)
(declare-fun input1_d17 () String)
(declare-fun input1_d18 () String)
(declare-fun input1_d19 () String)
(declare-fun input1_d20 () String)
(declare-fun input1_d21 () String)
(declare-fun input1_d22 () String)
(declare-fun input1_d23 () String)
(declare-fun input1_d24 () String)
(declare-fun input1_d25 () String)
(declare-fun input1_d26 () String)
(declare-fun input1_d27 () String)
(assert (= input3 (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ input3_d0 (str.++ "," input3_d1)) (str.++ "," input3_d2)) (str.++ "," input3_d3)) (str.++ "," input3_d4)) (str.++ "," input3_d5)) (str.++ "," input3_d6)) (str.++ "," input3_d7)) (str.++ "," input3_d8)) (str.++ "," input3_d9)) (str.++ "," input3_d10)) (str.++ "," input3_d11)) (str.++ "," input3_d12)) (str.++ "," input3_d13)) (str.++ "," input3_d14)) (str.++ "," input3_d15)) (str.++ "," input3_d16)) (str.++ "," input3_d17)) (str.++ "," input3_d18)) (str.++ "," input3_d19)) (str.++ "," input3_d20)) (str.++ "," input3_d21)))); splitHandler input3 22
(assert (= input6 (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ input6_d0 (str.++ "," input6_d1)) (str.++ "," input6_d2)) (str.++ "," input6_d3)) (str.++ "," input6_d4)) (str.++ "," input6_d5)) (str.++ "," input6_d6)) (str.++ "," input6_d7)) (str.++ "," input6_d8)) (str.++ "," input6_d9)) (str.++ "," input6_d10)) (str.++ "," input6_d11)) (str.++ "," input6_d12)) (str.++ "," input6_d13)) (str.++ "," input6_d14)) (str.++ "," input6_d15)) (str.++ "," input6_d16)) (str.++ "," input6_d17)) (str.++ "," input6_d18)) (str.++ "," input6_d19)) (str.++ "," input6_d20)) (str.++ "," input6_d21)) (str.++ "," input6_d22)) (str.++ "," input6_d23)) (str.++ "," input6_d24)) (str.++ "," input6_d25)) (str.++ "," input6_d26)) (str.++ "," input6_d27)) (str.++ "," input6_d28)))); splitHandler input6 29
(assert (= input2 (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ input2_d0 (str.++ "," input2_d1)) (str.++ "," input2_d2)) (str.++ "," input2_d3)) (str.++ "," input2_d4)) (str.++ "," input2_d5)) (str.++ "," input2_d6)) (str.++ "," input2_d7)) (str.++ "," input2_d8)) (str.++ "," input2_d9)) (str.++ "," input2_d10)) (str.++ "," input2_d11)) (str.++ "," input2_d12)) (str.++ "," input2_d13)) (str.++ "," input2_d14)) (str.++ "," input2_d15)) (str.++ "," input2_d16)) (str.++ "," input2_d17)) (str.++ "," input2_d18)) (str.++ "," input2_d19)) (str.++ "," input2_d20)) (str.++ "," input2_d21)) (str.++ "," input2_d22)))); splitHandler input2 23
(assert (= input5 (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ input5_d0 (str.++ "," input5_d1)) (str.++ "," input5_d2)) (str.++ "," input5_d3)) (str.++ "," input5_d4)) (str.++ "," input5_d5)) (str.++ "," input5_d6)) (str.++ "," input5_d7)) (str.++ "," input5_d8)) (str.++ "," input5_d9)) (str.++ "," input5_d10)) (str.++ "," input5_d11)) (str.++ "," input5_d12)))); splitHandler input5 13
(assert (= input4 (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ input4_d0 (str.++ "," input4_d1)) (str.++ "," input4_d2)) (str.++ "," input4_d3)) (str.++ "," input4_d4)) (str.++ "," input4_d5)) (str.++ "," input4_d6)) (str.++ "," input4_d7)) (str.++ "," input4_d8)) (str.++ "," input4_d9)) (str.++ "," input4_d10)) (str.++ "," input4_d11)) (str.++ "," input4_d12)) (str.++ "," input4_d13)) (str.++ "," input4_d14)) (str.++ "," input4_d15)) (str.++ "," input4_d16)) (str.++ "," input4_d17)))); splitHandler input4 18
(assert (= input1 (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ (str.++ input1_d0 (str.++ "," input1_d1)) (str.++ "," input1_d2)) (str.++ "," input1_d3)) (str.++ "," input1_d4)) (str.++ "," input1_d5)) (str.++ "," input1_d6)) (str.++ "," input1_d7)) (str.++ "," input1_d8)) (str.++ "," input1_d9)) (str.++ "," input1_d10)) (str.++ "," input1_d11)) (str.++ "," input1_d12)) (str.++ "," input1_d13)) (str.++ "," input1_d14)) (str.++ "," input1_d15)) (str.++ "," input1_d16)) (str.++ "," input1_d17)) (str.++ "," input1_d18)) (str.++ "," input1_d19)) (str.++ "," input1_d20)) (str.++ "," input1_d21)) (str.++ "," input1_d22)) (str.++ "," input1_d23)) (str.++ "," input1_d24)) (str.++ "," input1_d25)) (str.++ "," input1_d26)) (str.++ "," input1_d27)))); splitHandler input1 28
(assert (>= (str.len input5_d9) 5)) 
(assert (>= (str.len input6_d25) 5)) 
(assert  (and (=  x3 input1_d0 )  (and (=  x6 input2_d22 )  (and (=  x9 input2_d1 )  (and (=  x16 input3_d0 )  (and (=  x19 input2_d2 )  (and (=  x26 input4_d0 )  (and (=  x29 input4_d4 )  (and (=  x38 input5_d0 )  (and (=  x41 input2_d6 ) (=  x52 input6_d0 ) ) ) ) ) ) ) ) ) ) )
(assert  (and (=  ( str.substr input5_d9  0 (- 5 0) ) ( str.substr input6_d25  0 (- 5 0) ) )  (and (=  input1_d6  "1999"  )  (and (=  input1_d8  "11"  )  (and (=  x3 x6 )  (and (=  input3_d20  "50"  )  (and (=  x9 x16 )  (and (=  x19 x26 )  (and (=  x29 x38 ) (=  x41 x52 ) ) ) ) ) ) ) ) ) )
(check-sat)
(get-model)
