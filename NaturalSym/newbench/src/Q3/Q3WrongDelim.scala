...
object Q3WrongDelim extends SparkProgramTemplate {
  ...
  override def execute(input1: RDD[String], input2: RDD[String], input3: RDD[String]): RDD[_] = {
    ...
    val filter1 = date_dim.filter(row => row.split(",")(8).equals("11") /*d_moy*/)
    ...
    val map3 = join1.map {
      ...
      //case (date_sk, (date_dim_row, ss_row)) =>
        (ss_row.split(":")(1) /*ss_item_sk*/ , (date_dim_row, ss_row)) // wrong delim: ","
    }
