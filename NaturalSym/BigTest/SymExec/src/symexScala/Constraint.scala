  ...
  override def toZ3Query(initials: Z3QueryState): String = {
    ...
    try {
      if (leftExpr.isInstanceOf[ConcreteValue] && isString) { 
        //@thaddywu: weired logic, seems impossible condition to be as both StringExpr and ConcreteValue instance 
        //  update on June, 29: This logic seems to handle the problem of JPF's incapability of extracting out String Constant
        //  Now, we add support in the symbolic listener, which should be working.
        println("\u001B[31m Needs revision \u001B[0m");
    ...
    //  }
    // @thaddywu: optimization for isinteger, need revision
    /*
    if (op == UniaryOp.IsInteger && (left.isInstanceOf[SymVar] || 
    (left.isInstanceOf[StringExpr] && left.asInstanceOf[StringExpr].op.op.equals(StringOp.Splitn) && left.asInstanceOf[StringExpr].obj.isInstanceOf[SymVar]))) {
      // println("[Optimization] isinteger " + leftstr);
      val iname = leftstr.replaceAll("[^A-Za-z0-9_]","") + "i";
      initials.init.add((iname, Numeric(NumericUnderlyingType._Int)));
      initials.replacements(iname) = "( str.to.int " + leftstr + "  )" //( str.to.int  line5  )
      return s"""(= (int.to.str ${iname}) ${leftstr} )"""
    }
    */
...
class Clause(left: Expr, op: ComparisonOp = null, right: Expr = null) {
  ...
  def toZ3Query(initials: Z3QueryState): String = {
    var isString = (leftExpr.actualType == NonNumeric(NonNumericUnderlyingType._String) || right.actualType == NonNumeric(NonNumericUnderlyingType._String));
    ...
    if (compOp == null || rightExpr == null)
      ...
      if (compOp == Notequals || compOp == Inequality) {
        ...
        return s"""(${if (compOp == Equality || compOp == Equals) { //@thaddywu: bug here, but not changed
          "="
        } else {
          compOp.toString()
        }}  ${leftstr} ${rightstr} )"""
