...
class SymVar(atype: VType, var name: String) extends Terminal {
  ...
  override def applyEffect(x: SymVar, effect: Expr): Expr = {
    if (this.name.equals(x.name)) effect // @thaddywu
    else
  ...
  override def replace(thisVar: SymVar, other: SymVar): SymVar = {
    if (thisVar.getName.equals(this.getName)) other
    else this
  }
  ...
  override def equals(x: Any): Boolean = {
    if (x.isInstanceOf[SymVar])
      x.asInstanceOf[SymVar].name.equals(this.name)
    else false
  }
}
