...
public class JPF implements Runnable {
	...
	/**
	 * symbolic Listener object // Gulzar
	 * */
	SymbolicListener sym;
	PathEffectListenerImp pfl;
	...
	public static SymbolicResult computeSym(SymbolicState state, JPFDAGNode node, String modelDir) {
		if (node.operator_name.startsWith("input"))
			return new EntrySymbolicResult(state, node.operator_name);

		SymbolicResult[] results = new SymbolicResult[node.parents.size()];
		for (int i = 0; i < node.parents.size(); i++)
			results[i] = computeSym(state, node.parents.get(i), modelDir);
		
		System.out.println("\u001B[32m");
		System.out.println("==============================");
		String deps = "";
		for (JPFDAGNode p: node.parents)
			deps += p.operator_name + ", ";
		System.out.println("||" + node.operator_name + " -> " + deps);
		System.out.println("==============================");
		System.out.println("\u001B[0m");

		SymbolicResult udfResult = null;
		SymbolicResult ret = null;

		String operator_type = node.operator_name.replaceAll("[0-9]","");
		if ("map,filter,reduce,reduceByKey,flatMap".contains(operator_type)) {
			String args_new[] = new String[]{modelDir + node.operator_name + ".jpf"};
			Config conf1 = createConfig(args_new);
			JPF jpf = new JPF(conf1);
			jpf.run();
			udfResult = jpf.pfl.convertAll(state, node.operator_name);
			/* 
			System.out.print("Before pruning: ");
			System.out.println(udfResult.numOfPaths() + " " + udfResult.numOfTerminating());
			udfResult = udfResult.filterInvalidPath(true);
			System.out.print("After pruning: ");
			System.out.println(udfResult.numOfPaths() + " " + udfResult.numOfTerminating());
			*/
			
			// udfResult.concludes(node.operator_name, true);
		}
		if (operator_type.equals("map"))
			ret = results[0].map(udfResult);
		if (operator_type.equals("filter"))
			ret = results[0].filter(udfResult);
		if (operator_type.equals("reduce"))
			ret = results[0].reduce(udfResult);
		if (operator_type.equals("reduceByKey"))
			ret = results[0].reduceByKey(udfResult);
		if (operator_type.equals("flatMap"))
			ret = results[0].flatMap(udfResult);

		if (operator_type.equals("join"))
			ret = results[0].join(results[1]);
		if (operator_type.equals("groupByKey"))
			ret = results[0].groupByKey();
		
		ret.setKeyLength(node.keyLength);
		 // @thaddywu, Sep 3 2023
		 // Workaround to store the shape of output.
		
		if (! "map,filter,reduce,reduceByKey,flatMap,join,groupByKey".contains(operator_type))
			throw new RuntimeException("This data flow operation is yet not supported! :" + operator_type);

		// ret.concludes(node.operator_name + "@applyed", true);
		return ret;
	}
	...
	public static void runJPFWithBtest(String[] args) {
		if(args[0].equals("-enableBT")) {
			...
			SymbolicResult summary = computeSym(symState , node ,  modelDir);
			long tool_time  =  System.currentTimeMillis();

			// summary.concludes("final", true);
			summary.report();
			
			long end_time = System.currentTimeMillis();
			System.out.println("Total Test Generation Time : " + (end_time - start) + " ms");
			System.out.println("Total Solver Time : " + (end_time - tool_time) + " ms");
			System.out.println("Total Constraint generation Time: " + (tool_time - start) + " ms");

			if (args.length >= 3) {
				// write stats into args[2]
				try {
					FileWriter writer = new FileWriter(args[2]);
					BufferedWriter buffer = new BufferedWriter(writer);
					buffer.write((end_time - start) + "\n");
					buffer.write((end_time - tool_time) + "\n");
					buffer.write((tool_time - start) + "\n");
					buffer.close();
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	public static void runUDF(String[] args) {
		long start = System.currentTimeMillis();
		String modelDir = Runner.getModelPath();
		...
		for (int i = 1; i < args.length; i++) {
			...
			args_new[0] = modelDir + args[i] + ".jpf";
			Config conf1 = createConfig(args_new);
			JPF jpf = new JPF(conf1);
			
			jpf.run();
			SymbolicResult udfResult = jpf.pfl.convertAll(symState, args[i]);
			udfResult.concludes(args[i], true);
		}
	...
	public static void start(Config conf, String[] args){
		...
		if (shell != null) {
			...
			try {
				if (args[0].equals("-enableBT")) {// ----->  Running BigTest 
					...
					jpf.run();
					System.out.println("Done Running JPf");
				}
	...
	void addListeners () {
		...
		for(JPFListener list: listeners) {
			if(list instanceof SymbolicListener) {
				sym = (SymbolicListener) list;
			}
		}
		/**
		 * Edits by Gulzar // @thaddywu reviewed
		 * */

		PathEffectListenerImp injectedListener = new PathEffectListenerImp();
		pfl = injectedListener;
		listeners.add(new SymbolicListener(config, this, injectedListener));
