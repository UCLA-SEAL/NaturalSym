...
object MutantGenerator extends Transformer {
  ...
  def getAllBinOpsPositions(tree: Tree): List[Term.ApplyInfix] = {
    tree match {
      case node @ Term.ApplyInfix(lhs,op,targs,args) if List(BOOLEAN, ARITHMETIC).contains(getOpType(op)) =>
        // println(lhs + " :op " + op + " :targs:" + targs.mkString(",") + "\nargs: " + args.mkString(","))
        if (lhs.toString.contains('\"') || args.toString.contains('\"')) List[Term.ApplyInfix]()
        else
